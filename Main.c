...
int gl_nLaserCurrentUnstableT2 = 0;   //последний момент времени когда была просадка тока разряда (отслеживается, и если за последние 5 сек просадок не было - сбрасывается)
int gl_nLaserCurrentUnstableCnt = 0;  //счётчик просадок тока в течении последних 5 сек
int gl_nVibrationWatchT2 = 0;         //последний момент времени когда была просадка амплитуды колебаний (отслеживается, и если за последние 5 сек просадок не было - сбрасывается)
...
/*#ifdef DEBUG
  ...
  //while ((T2VAL == nT2VALold) || (T2VAL > 3)); // ensures timer is started...
...
*/
  ...
  //засекаем момент последнего прихода такт.сигнала
  n_prT2VAL = T2VAL;
    ...
    */
    ...
    if( GP0DAT & 0x10) {  //на ноге P0.4 есть сигнал
      ...
      if( gl_b_SA_Processed == 0) { //если в этом SA цикле мы его еще не обрабатывали
        ...
        // ************************************************************************************
        if( gl_bManualLaserOff == 0 && nSentPacksCounter == 6) {
        ...
        if( nRppTimer != 0) {
          if( gl_b_PerimeterReset == 1) {
            if( (( T2LD + nRppTimer - T2VAL) % T2LD) > 32768 * 0.1) {    //длительность верхней части RESET-сигнала 0.100 s
            ...
            if( (( T2LD + nRppTimer - T2VAL) % T2LD) > 32768 * 0.1) {    //длительность нижней части RESET-сигнала 0.100 s
